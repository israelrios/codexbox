...
```json
{
  ...
  "allow_var_patterns": ["CUSTOM_PUBLIC_*"],
  "directory_rules": [
    {
      "path": "~/work/project",
...
- `approved_paths` stores one-time approvals for env-derived read-only file and directory mounts.
- `approved_socket_vars` stores one-time approvals for env vars whose values resolve to Unix sockets.
...
- `SSH_AUTH_SOCK` is forwarded by default; if it points to an existing Unix socket and you approve `SSH_AUTH_SOCK`,
  codexbox bind-mounts the current socket path into the sandbox so SSH agent forwarding keeps working when the socket
  pathname changes.
...
- If `~/.ssh/known_hosts` exists on the host, `codexbox` bind-mounts it read-only to a seed path and copies it into the
...
- File and directory approvals are stored by path; socket approvals are stored by env var name.
